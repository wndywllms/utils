...
def cube_extract(f,ra,dec,x,y,size,size2=None,hduid=0,verbose=True):
    """
...
def extract_subim(filename,ra,dec,size,size2=None,hduid=0,verbose=True,cubemode=False):
    ...
    y=imc[0][1]
    if cubemode:
        hdu=cube_extract(orighdu,ra,dec,x,y,psize,psize2,hduid=hduid,verbose=verbose)
    else:
        hdu=flatten(orighdu,ra,dec,x,y,psize,psize2,hduid=hduid,verbose=verbose)
    return hdu

def extract_and_save(filename,ra,dec,size,size2=None,outname='cutout.fits',cubemode=False,verbose=False):
    if size2 is None:
        size2 = size
    hdu=extract_subim(filename,ra,dec,size,size2,verbose=verbose,cubemode=cubemode)
